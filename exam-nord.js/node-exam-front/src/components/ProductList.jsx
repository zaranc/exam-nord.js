...
const ProductList = () => {
  ...
  const getproductdata = async () => {
    try {
      const res = await axios.get(`http://localhost:3001/v1/product/getproduct`);
  ...
  return (
    <div className="grid grid-cols-3 gap-5">
      {product?.map((val) => (
        <div
          key={val._id}
          className="flex flex-col justify-between text-white gap-2 max-w-sm p-7 bg-slate-800 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"
        >
          <a href="#">
